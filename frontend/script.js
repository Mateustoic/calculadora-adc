        ...
        return `
            <div class="disciplina-card ${cores.classe}" onclick="mostrarFaltas(${disciplina.id})">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 0.5rem;">
                    <p class="${cores.texto}" style="margin: 0;">${diasRestantes} ${texto} (${totalFaltas} faltas)</p>
                    <button class="btn-delete-card" onclick="event.stopPropagation(); excluirDisciplina(${disciplina.id})" style="margin-left: 1rem;">
...
function excluirDisciplina(disciplinaId) {
    ...
    const faltas = JSON.parse(localStorage.getItem('faltas') || '[]');
    const novasFaltas = faltas.filter(f => f.disciplinaId !== disciplinaId);
    localStorage.setItem('faltas', JSON.stringify(novasFaltas));
